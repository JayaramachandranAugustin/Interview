...
In this lesson we will see about a interesting problem, Which is Minimum adjacent swaps to convert a string to palindrome. Palindrome is a sequence of characters which reads same as forward and backward. Example is NOON which reads same from forward and backward.
...
Lets See can we make this string as palindrome. Since we know palindrome reads same from forward and backward, So every characters except one in case of odd number of characters must be in pairs. What that mean! Every character except one need to be completely divisible by 2.
